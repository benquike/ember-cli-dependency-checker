...
class EmberCLIDependencyChecker {
  ...
  lookupBowerPackageVersion(name) {
    ...
    if (!this.project.hasOwnProperty("bowerDirectory")) {
      const bowerConfigPath = path.join(this.project.root, '.bowerrc');
      const bowerConfigJson = fs.existsSync(bowerConfigPath) ? fs.readJsonSync(bowerConfigPath) : {};
      this.project.bowerDirectory = bowerConfigJson.directory || 'bower_components';
    }
  ...
  normalizedBowerDependencies() {
    ...
    if (fs.existsSync(bowerPath)) {
      ...
      // eslint-disable-next-line no-prototype-builtins
      if (!this.project.hasOwnProperty("bowerDependencies")) {
